...
static int cam_hyp_intf_open(struct inode *inode, struct file *filp)
{
	...
	cam_hyp_intf_dev->ionc = msm_ion_client_create(
			CAM_HYP_INTF_DRIVER_NAME);
	if (IS_ERR_OR_NULL(cam_hyp_intf_dev->ionc)) {
		rc = -ENOMEM;
		goto err_ion_fail;
	}
...
fail_rx_ready:
	ion_client_destroy(cam_hyp_intf_dev->ionc);
err_ion_fail:
	atomic_dec(&cam_hyp_intf_dev->open_cnt);
err_busy:
	mutex_unlock(&cam_hyp_intf_dev->hyp_intf_lock);
	...
	struct res_mgr_msg **msg, size_t *size)
{
	...
	handle = ion_import_dma_buf_fd(dev->ionc, fd);
	if (IS_ERR_OR_NULL(handle)) {
		CAM_ERR(CAM_HYP, "Could not get ion handle(error %lu)",
			(unsigned long)handle);
		return -EINVAL;
	}

	sg_table = ion_sg_table(dev->ionc, handle);
	if (IS_ERR_OR_NULL(sg_table)) {
		CAM_ERR(CAM_HYP, "Could not get sg table (error %lu)",
