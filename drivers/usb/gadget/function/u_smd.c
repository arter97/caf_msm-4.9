...
static unsigned int n_smd_ports;
u32			extra_sz;
...
static struct usb_request *gsmd_alloc_req(struct usb_ep *ep, unsigned int len,
...
static void gsmd_start_rx(struct gsmd_port *port)
{
	...
	while (test_bit(CH_OPENED, &port->pi->flags) && !list_empty(pool)) {
		...
		spin_lock_irqsave(&port->port_lock, flags);
		if (ret) {
			pr_err("%s: usb ep out queue failed port:%pK, port#%d\n",
...
static void gsmd_rx_push(struct work_struct *w)
{
	...
	while (pi->ch && !list_empty(q)) {
		...
		case -ESHUTDOWN:
			...
			goto rx_push_end;
		default:
			pr_warn("%s: port:%pK port#%d Unexpected Rx Status:%d\n",
				__func__, port, port->port_num, req->status);
		/* fall through */
		case 0:
			/* normal completion */
			break;
...
static void gsmd_tx_pull(struct work_struct *w)
{
	...
	while (pi->ch && !list_empty(pool)) {
		...
		spin_lock_irq(&port->port_lock);
		if (ret) {
			pr_err("%s: usb ep in queue failed port:%pK, port#%d err:%d\n",
...
static void gsmd_write_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->status)
		pr_warn("%s: port:%pK port#%d unexpected %s status %d\n",
				__func__, port, port->port_num,
				ep->name, req->status);
...
static void gsmd_notify(void *priv, unsigned int event)
{
...
void gsmd_cleanup(struct usb_gadget *g, unsigned int count)
{
	/* TBD */
}
